...
/////
	...
	const getData = async (url) =>{
		const res = await fetch(url);
		//как уже говорилось, для фетча коды 404 и 505 не явл ошибкой. Отсутствие интернета- вот ошибка.
		//поэтому их нукжно вручную обработать

			//запомни то че ниже
		if(!res.ok){
			throw new Error(`smth went wrong, status ${res.status}`);
		}
